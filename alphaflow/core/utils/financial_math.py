...
# ==========================================
# 5. 币种修正市值解析 (Currency-Aligned Market Cap)
# ==========================================

def resolve_aligned_mcap(
    mcap_rmb: Optional[float],
    mcap_raw: Optional[float],
    currency_ctx: Optional[Dict[str, Any]],
) -> Optional[float]:
    """
    返回与财报同币种的市值，供跨币种估值指标（如 PS_TTM）使用。

    Fallback 链（遵守 .clinerules Section VII）：
      P1: MARKET_CAP_RMB  — Collector 预折算值（最优汇率）
      P2: alignment_factor — 东财 PE 对撞隐含的报告期加权平均汇率
      P3: FX_RATE          — 实时汇率（有 2-5% 时间差偏移）
      P4: None             — 无法安全修正，优雅降级

    无错配时直接返回原始市值（分子分母已同币种）。
    """
    # P1: Collector 预折算值
    if mcap_rmb is not None and mcap_rmb > 0:
        return mcap_rmb
    ...
    # 无错配 → 原始值即同币种
    if not currency_ctx or not currency_ctx.get("is_misaligned"):
        return mcap_raw

    # P2: alignment_factor（报告期加权平均汇率）
    factor = currency_ctx.get("alignment_factor")
    if factor is not None and factor != 1.0 and factor > 0:
        return mcap_raw * factor

    # P3: FX_RATE（实时汇率）
    fx_rate = currency_ctx.get("fx_rate")
    if fx_rate is not None and fx_rate > 0:
        return mcap_raw * fx_rate
