...
class ResearchPackFacade:
    ...
    def __init__(self, pack: ResearchPack):
        ...
        self._analysis_series: Dict[str, List[Dict]] = pack.extra.get(
            "akshare_analysis", {}
        )
        ...
        self._domain_routers = {
            ...
            "balance": lambda p, k: self._resolve_statement(
                p, StatementType.BALANCE, k
            ),
            "cash": lambda p, k: self._resolve_statement(p, StatementType.CASH, k),
            "analysis": lambda p, k: self._resolve_statement(
                p, StatementType.ANALYSIS, k
            ),
        }
    ...
    def get_scoped_series(
        self, period: str, stmt_type: StatementType = StatementType.INCOME
    ) -> List[Dict]:
        """
        获取指定周期和类型的财务数据序列
    ...
    def get_ttm_value(
        ...
        Args:
            offset_years: 时空偏移量。0=当前TTM，1=去年同期TTM (TTM_PREV)，
                          2=两年前同期TTM。参数化设计，符合 DRY 原则。
        """
        stmt_key = self.STMT_KEY_MAP[stmt_type]
        ...
        # 🚀 时空偏移切片：当 offset_years > 0 时，将序列截断到历史锚点
        if offset_years > 0:
            # 锚点来源：优先季报序列头，降级到年报序列头
            anchor_src = (q_series[0] if q_series else a_series[0] if a_series else None)
            if not anchor_src or not anchor_src.get(MetaKey.PERIOD_ENDING):
                return None
            anchor = pd.to_datetime(anchor_src[MetaKey.PERIOD_ENDING]) - pd.DateOffset(
                years=offset_years
            )
            q_series = [
                q
                for q in q_series
                if pd.to_datetime(q.get(MetaKey.PERIOD_ENDING, "1900-01-01")) <= anchor
            ]
            a_series = [
                a
                for a in a_series
                if pd.to_datetime(a.get(MetaKey.PERIOD_ENDING, "1900-01-01")) <= anchor
            ]
            if not q_series and not a_series:
                return None
    ...
    def get_baseline_context(self) -> Tuple[str, Optional[datetime], Optional[Dict]]:
        ...
        获取基准上下文 (严格按照日期最新原则)
        """
        q_is = self.get_latest_report(
            ReportPeriod.QUARTERLY.value, StatementType.INCOME
        )
        a_is = self.get_latest_report(ReportPeriod.ANNUAL.value, StatementType.INCOME)

        q_date = (
            pd.to_datetime(q_is.get(MetaKey.PERIOD_ENDING))
            if q_is and q_is.get(MetaKey.PERIOD_ENDING)
            else pd.Timestamp.min
        )
        a_date = (
            pd.to_datetime(a_is.get(MetaKey.PERIOD_ENDING))
            if a_is and a_is.get(MetaKey.PERIOD_ENDING)
            else pd.Timestamp.min
        )
    ...
    # ======================================
    # V4 架构：字典路由式依赖解析器 (OCP-Compliant)
    # 新增锚点只需在 _build_period_router 中注册一行，无需修改解析逻辑
    # ======================================

    def _build_period_router(self) -> Dict[str, Callable]:
        """构建时态路由表 — 新增锚点只需注册，无需修改解析逻辑 (OCP)"""
        return {
            "TTM": lambda k, s: self.get_ttm_value(k, stmt_type=s, offset_years=0),
            "TTM_PREV": lambda k, s: self.get_ttm_value(k, stmt_type=s, offset_years=1),
            "LATEST": lambda k, s: self.get_snapshot_value(
                k, stmt_type=s, period="latest"
            ),
            "AVERAGE_1Y": lambda k, s: self._resolve_average_1y(k, s),
            "ANNUAL_LATEST": lambda k, s: self._resolve_annual(k, s, index=0),
            "ANNUAL_PREV": lambda k, s: self._resolve_annual(k, s, index=1),
            "ANNUAL": lambda k, s: self.get_snapshot_value(
                k, stmt_type=s, period="annual"
            ),
            "QUARTERLY": lambda k, s: self.get_snapshot_value(
                k, stmt_type=s, period="quarterly"
            ),
            "SEMIANNUAL_LATEST": lambda k, s: self._resolve_semiannual(k, s),
        }

    def _resolve_statement(
        self, period: str, stmt_type: StatementType, standard_key: str
    ) -> Optional[float]:
        """内部辅助方法：处理结构化财报的时态路由（字典查表，O(1)）"""
        router = self._build_period_router().get(period)
        return router(standard_key, stmt_type) if router else None

    # --- 时态路由辅助方法 (从 elif 链中提取) ---

    def _resolve_annual(
        self, standard_key: str, stmt_type: StatementType, index: int = 0
    ) -> Optional[float]:
        """按索引提取年报数据：index=0 最新，index=1 上一年"""
        series = self.get_scoped_series("annual", stmt_type)
        return (
            get_field_value(series[index], standard_key)
            if len(series) > index
            else None
        )

    def _resolve_semiannual(
        self, standard_key: str, stmt_type: StatementType
    ) -> Optional[float]:
        """定位最新半年报 (REPORT_TYPE == 'semiannual')"""
        q_series = self.get_scoped_series("quarterly", stmt_type)
        for record in q_series:
            if record.get("REPORT_TYPE", "") == "semiannual":
                return get_field_value(record, standard_key)
        return None

    def _resolve_average_1y(
        self, standard_key: str, stmt_type: StatementType
    ) -> Optional[float]:
        """近一年平均值：(当前快照 + 一年前快照) / 2"""
        v_cur = self.get_snapshot_value(
            standard_key, stmt_type=stmt_type, period="latest"
        )
        v_prev = None
        q_series = self.get_scoped_series("quarterly", stmt_type)
        if q_series and len(q_series) > 0:
            cur_dt = q_series[0].get(MetaKey.PERIOD_ENDING)
            if cur_dt:
                ...
                if not prev_rep:
                    ...
                    if a_series:
                        prev_rep = (
                            a_series[0]
                            if a_series[0].get(MetaKey.PERIOD_ENDING) != cur_dt
                            else (a_series[1] if len(a_series) > 1 else None)
                        )
                if prev_rep:
    ...
    def resolve_dependency(
        self, period_type: str, domain: str, standard_key: str
    ) -> Optional[float]:
        """
