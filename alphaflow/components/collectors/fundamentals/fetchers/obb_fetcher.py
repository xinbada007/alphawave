...
class OBBFetcher(BaseFetcher):
        ...
        if task_name in self.TASK_CONFIG:
            ...
            return await self._exec_obb_task(
                ...
                **{
                    ...
                    if k not in ("func", "sort_key", "provider", "filter_fn")
                },
            )
        ...
        # 3. 财务报表任务 (动态映射：a_income, q_balance 等)
            ...
            if len(parts) >= 2 and parts[1] in ("income", "balance", "cash"):
                ...
                if hasattr(obb_any.equity.fundamental, stmt_key):
                    ...
                    return await self._exec_obb_task(
                        func, symbol, task_name, period=period, limit=limit
                    )
        ...
        async with self._semaphore:
            try:
                res = await asyncio.to_thread(
                    func, symbol=symbol, provider=target_provider, **kwargs
                )
    ...
    async def _fetch_earnings_cal(
        self, symbol: str, provider: str, limit: int = 8
    ) -> List[Dict]:
        ...
        try:
            ...
            if data:
                ...
                # 按日期倒序 (最新在前)
                data.sort(
                    key=lambda x: str(x.get("report_date", x.get("date", ""))),
                    reverse=True,
                )
                return data[:limit]
